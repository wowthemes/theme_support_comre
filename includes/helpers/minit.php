<?php
	...
	class Minit {
		...
		private function __construct() {
			...
			//if( !class_exists('force_gzip') ) require_once '../../../../plugins/theme_support_comre/includes/helpers/gzip.php';

			if(extension_loaded("zlib") && (ini_get("output_handler") != "ob_gzhandler")) {
				//ob_start('ob_gzhandler');
    			//add_action('wp', create_function('', 'ob_end_flush();ini_set("zlib.output_compression", 1);'));
			}
			$gzip_option = get_option('_sh_gzip_compression');
			//echo home_url().'.htaccess';exit;

			if(!$gzip_option){
				include_once '../../../../plugins/theme_support_comre/includes/helpers/gzip.php';

				$status = _sh_GZipNinjaSpeed_write_file(ABSPATH.'.htaccess');
				update_option('_sh_gzip_compression', true);
			}
		...
		function init_minit_js( $todo ) {
			...
			global $wp_scripts;
			if (($key = array_search('jquery', $todo)) !== false) {
			    array_splice($todo, $key, 1);
			}
			//printr($todo);
			printr($this->minit_objects( $wp_scripts, $todo, 'js' ));
		...
		function minit_objects( &$object, $todo, $extension ) {
			...
			if ( ! is_dir( $wp_upload_dir['basedir'] . '/wowthemes' ) )
				if ( ! mkdir( $wp_upload_dir['basedir'] . '/wowthemes' ) )
					return $todo;

			$combined_file_path = sprintf( '%s/wowthemes/%s.%s', $wp_upload_dir['basedir'], $cache_ver, $extension );
			$combined_file_url = sprintf( '%s/wowthemes/%s.%s', $wp_upload_dir['baseurl'], $cache_ver, $extension );
			...
			switch ( $extension ) {
				...
				case 'js':
					...
					wp_enqueue_script(
						...
						null,
						null,
						apply_filters( 'minit-js-in-footer', false )
					);
	...
	function minit_cache_delete_files() {
		...
		$wp_upload_dir = wp_upload_dir();
		$minit_files = glob( $wp_upload_dir['basedir'] . '/wowthemes/*' );
	...
	{
	    if ( FALSE === strstr($url, '/wowthemes/') || !strstr($url, '.js') )
	    { 
		    // not our file
		    return $url;
	    }
